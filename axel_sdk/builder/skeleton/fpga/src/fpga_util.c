...
int fpga_init(void) {
  ...
  status = ADMXRC2_SetClockRate(card, ADMXRC2_CLOCK_LCLK, LCLK_FREQ*1e6, NULL);
  if (status != ADMXRC2_SUCCESS) {
  ...
  status = ADMXRC2_SetClockRate(card, 1, MCLK_FREQ*1e6, NULL);
  if (status != ADMXRC2_SUCCESS) {
    printf("Failed to set LCLK to %.1fMHz: %s\n",
...
int fpga_dmawrite(void *buf, int size, int addr) {
  ADMXRC2_STATUS     status;
  int bank, page;
  int local_bus_addr;
  ...
  while (size > 0) {
    ...
    bank =  addr / 0x20000000;
    page = (addr & 0x1fffffff) / 0x200000;
    ...
    status = ADMXRC2_DoDMAImmediate(card, 
        buf, size<WINDOW_SIZE?size:WINDOW_SIZE, local_bus_addr,
...
int fpga_dmaread(void *buf, int size, int addr) {
  ADMXRC2_STATUS     status;
  int bank, page;
  int local_bus_addr;
